...
| --- | --- | --- |
| clean | `0` | Nothing found, and the target was fully scanned. |
| suspicious | `0` | Heuristic findings only. Read them; nothing was changed. |
| infected | `1` | At least one confirmed finding. |
| error | `2` | The target could not be fully scanned — unknown, not clean. |
...
Known CVEs in your dependencies are reported alongside findings and **never change the verdict or the
exit code**. A vulnerable dependency is a thing to plan, not an infection to contain, and folding the
two together would make the gate meaningless in both directions. See [the advisory
...
machine is safe, and exits `3` when it is not — see [audit a
