...
| `2` | Usage error (unknown command, bad option, a missing explicit `--config` path), **or** a run that could not complete — a malformed config, or a target that errored while being scanned. A target `saw` could not scan is never reported as clean. `saw audit` exits `2` when a check could not run: its own self-test failed, or it stopped with an error. A check that did not answer has not established the surface it covers, so its quiet is not a clean result. `saw guard setup` also exits `2` when it cannot resolve the release SHA (offline → pass `--ref`). |
