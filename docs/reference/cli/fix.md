...
| `amend` | Replace past commits that still carry the payload and force-update each branch they sat on. That force-update is the fix. Tags and notes are not moved. If the remote does not move, local branches are left as they stood. The previous objects remain until collected. |
