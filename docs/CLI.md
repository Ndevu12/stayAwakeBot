...
| [`saw doctor`](#saw-doctor) | Self-check: install resolution + credentials + intent capabilities | — |
...
| `-j`, `--jobs N` | Scan **concurrently** — up to N repos at once in a multi-repo sweep, **and** split a single big repo/monorepo/vendored tree across N workers by file. **Default AUTO:** a small scan runs sequentially (no overhead), a big one uses one worker per CPU core. Pass a number to cap it, `-j 1` to force **sequential** (reproducible / low-load runs), or `auto`. Config `settings.jobs` sets the default; `settings.parallel_min_files` sets the file-count floor below which a single target stays sequential. Workers are **processes** (local scanning is CPU-bound; Python's GIL means threads wouldn't help). Detection is unchanged and a persisted report (`-d`/`--json`/`--sarif`) is **byte-identical** to a sequential run; a worker that dies is reported as an ERROR and still fails **closed** (exit 2). |
