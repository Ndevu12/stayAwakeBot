...
def _backup_write_verify(root: Path, rel: str, new_text: str, rollback: Path, content_sig) -> bool:
...
def _apply_seam_excision(root: Path, rel: str, expected: str, rollback: Path, content_sig) -> bool:
    ...
    if _seam_strip(current, _ext(rel), content_sig) != expected:
        return False
    return _backup_write_verify(root, rel, expected, rollback, content_sig)


def _apply_whole_file_restore(root: Path, rel: str, expected: str, rollback: Path, content_sig) -> bool:
...
def apply_recovery(repo, rec: Recovery, rollback: Path, content_sig) -> bool:
    """Write `rec.clean_text` after backing up. Revert if the write does not verify."""
    root = Path(repo)
    if rec.excised:
        return _apply_seam_excision(root, rec.path, rec.clean_text, rollback, content_sig)
    ...
    if not _is_subsequence(rec.clean_text, current):
        return False
    return _backup_write_verify(root, rec.path, rec.clean_text, rollback, content_sig)


def apply_suggested(repo, sug: "Suggested", rollback: Path, content_sig) -> bool:
    """Write a computed strip or restore."""
    if getattr(sug, "apply_mode", "seam") == "restore":
        return _apply_whole_file_restore(Path(repo), sug.path, sug.excised_text, rollback, content_sig)
    return _apply_seam_excision(Path(repo), sug.path, sug.excised_text, rollback, content_sig)
