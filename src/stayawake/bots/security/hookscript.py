...
def repository_hooks_dir(repo: Path) -> Path | None:
    ...
    answer = gitutil.stdout(repo, ["rev-parse", "--git-path", "hooks"]).strip()
    if not answer:
        return None
...
def in_managed_dir(path: Path) -> bool:
    ...
    if not _same(path.parent, hooks_dir()):
        return False
    adopted = global_hooks_path()
    return not (adopted and _same(Path(os.path.expanduser(adopted)), hooks_dir()))
