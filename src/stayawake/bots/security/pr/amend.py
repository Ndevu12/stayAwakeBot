#!/usr/bin/env python3
"""`saw fix amend` — replace past commits that still carry the payload.
...
def amend_repo(repo: Path, opts, signatures, allowlist) -> str:
    """Replace one confirmed past commit on the current branch. Returns the operator line."""
    ...
    if len(merges) > 1:
        ...
        return (f"{display}: {len(merges)} confirmed past commits on this branch "
                f"— not replaced ({shas})")
    ...
    tag_note = ("; tags still point at the previous commit"
                if tags else "")
    return (f"{display}: replaced commit {merge[:12]} on '{branch}' ({after}). "
