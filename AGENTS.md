...
summary; Cursor reads the same text as `AGENTS.md`. The detail lives in **`.claude/skills/`** (Claude
Code) and **`.cursor/skills/`** (Cursor) — keep the two trees in lockstep — and in `docs/`. Read the
...
**`saw-overview`** skill and `docs/reference/cli/index.md`.
...
`.claude/skills/README.md` or `.cursor/skills/README.md`, which also state the rule these follow:
**skills publish obligations, never detection mechanisms.** The two skill trees are the same content
