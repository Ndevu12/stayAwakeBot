...
| **Remediate** | `saw fix` — prepares a cleanup branch per infected repo, `--pr` to publish. `saw fix amend` replaces past commits that still carry the payload (local; does not publish). `saw discard` undoes the branch/PR path. |
