...
1. **Zero downgrade in the scanner.** A missed detection is worse than a slow scan. Detection changes
   and perf/refactor work must be **byte-identical in findings** (prove it — diff vs `main`), and
   detectors are **tightened, never downgraded**. → `security-change-discipline`.
...
4. **`pin-bump-deferred` label on ANY PR touching `src/stayawake/bots/security/**`** (unless it bumps
   `sentinel-ref`) — the required `pin-freshness` check fails without it. Add it at `gh pr create`.
5. **Adversarial-verify security-critical changes and gate the push** — parallel refuters, lens matched
   to the risk (refute-safety / FP-hunt / FN-hunt / under-alarm), re-verify until clean.
6. **Respect the layers** `utils→lib→core→bots→cli` (import down-only; inject callables up).
   → `saw-architecture`.
7. **Offline & trust model:** accurate with zero flags; only `-x/--external` leaves the sandbox; the
   allowlist is operator-owned and never trusts a scanned repo's own config; fail closed. →
   `saw-overview`.
...
`saw-overview` · `saw-architecture` · `engineering-standard` · `working-with-this-codebase` ·
`shipping-changes` · `security-change-discipline` · `scanner-performance` ·
`security-hardening-patterns` — see `.claude/skills/README.md`.
