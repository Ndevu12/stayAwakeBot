...
- **`saw fix amend` removes the files a poisoned merge introduced alongside a payload, not only
  the payload itself.** When a merge introduced a confirmed payload into a directory it created whole
  — every file there absent from both branches it merged — those files are removed with the payload,
  and the removed paths are named in the result. A payload edited into an existing file is removed by
  restoring that file to its pre-payload content. A file the merge brought in from the branch it
  merges, one it merely edited, and a new file it added to an existing directory are kept. A file
  your own history already holds is never deleted — even one a merge re-added carrying a payload;
  when the payload cannot be lifted cleanly, the run reports that file for you to recover instead of
  removing it.
...
- **`saw harden` puts the scan-on-clone hooks in place as part of hardening.** You no longer have
  ...
  run says the machine is not fully protected rather than passing quietly. A hook that has been
  changed under you is still put back, and the run says so — leaving an unchanged hook alone is not
  the same as leaving a changed one alone.
...
- **A cleanup that did not finish names what is still there.** Where a confirmed fix could not
  remove something, the run reported the part that succeeded and said nothing about the rest, so an
  incomplete cleanup read as a finished one. A run that did finish says nothing.
...
- **A dangerous built-in is recognised by its name as written.** A longer name that merely ends
  in one is no longer reported as a dynamic-execution sink.
- **A renamed dangerous built-in is reported however far from the rename it is called.**
