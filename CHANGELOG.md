...
- **Maintainability: `bots/security/remediation.py` (802 lines) is now a `remediation/` package** split
  into `changes` (structure-safe transforms: quarantine, exact-line / JSON-key removal) and `recovery`
  (code-loader git-recovery / gated seam-excision / defer). Pure refactor: the moved code is
  byte-identical to the original (each section diffs clean), the package re-exports the flat public API
  (including the safety-gate helpers and defer-reason constants callers use), and the full suite passes
  **with zero test edits** (no behavior change).
