...
- **`saw fix amend` replaces past commits that still carry the payload and force-updates
  ...
  Tags and notes are not moved. If the remote does not move, local branches are left as they
  stood. The previous objects remain until collected. Bare `saw fix` still only cleans the tip.
